...
- (void) goToMediaShareViewWithMediaPath:(NSString*)mPath {
    ...
    if(mPath == nil || [mPath isEqualToString:@""]) {
        [commonUtils showVAlertSimple:@"Warning" body:@"Please take your video before proceeding." duration:1.2f];
        return;
    }

    [self doPostMedia:mPath];
...
- (void)doPostMedia:(NSString *)mPath {
    ...
    [paramDic setObject:@"2" forKey:@"bark_type"];
    [paramDic setObject:[commonUtils encodeMediaPathToBase64String:mPath] forKey:@"bark_video"];
    [paramDic setObject:[commonUtils encodeToBase64String:[self firstFrameOfVideoWithPath:mPath] byCompressionRatio:0.3] forKey:@"bark_video_thumb"];
    
    //[self requestAPI:paramDic];
    
}

- (UIImage *)firstFrameOfVideoWithPath:(NSString *)mPath {
    AVAsset *asset = [AVAsset assetWithURL:[NSURL fileURLWithPath:mPath]];
    AVAssetImageGenerator *imageGenerator = [[AVAssetImageGenerator alloc]initWithAsset:asset];
    imageGenerator.appliesPreferredTrackTransform = YES;
    CMTime time = CMTimeMake(1, 1);
    CGImageRef imageRef = [imageGenerator copyCGImageAtTime:time actualTime:NULL error:NULL];
