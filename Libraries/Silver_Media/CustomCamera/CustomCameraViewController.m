...
@property (weak, nonatomic) IBOutlet UIView *previewView, *containerView;
@property (weak, nonatomic) IBOutlet UIView *loadingView;
@property (weak, nonatomic) IBOutlet UIProgressView *progressBar;
@property (weak, nonatomic) IBOutlet UIImageView *captureButtonView;
@property (weak, nonatomic) IBOutlet UILabel *instructionLabel;
@property (weak, nonatomic) IBOutlet UIImageView *recordImageView, *gridImageView;
...
//    containerFrame.size.width = [UIScreen mainScreen].bounds.size.width;
//    [self.containerView setFrame:containerFrame];
    ...
    // Silver Media Change
    NSString *imageUrl = [commonUtils getFullPhotoUrl:[appController.currentUser objectForKey:@"user_photo_url"] withType:@"user"];
    [commonUtils setRoundedRectBorderButton:self.libraryButton withBorderWidth:1.0f withBorderColor:[UIColor whiteColor] withBorderRadius:4.0f];
    [commonUtils setButtonImageAFNetworking:self.libraryButton withImageUrl:imageUrl withPlaceholderImage:nil];
    
    
    CGRect previewViewFrame = self.previewView.frame;
    previewViewFrame.size.width = [UIScreen mainScreen].bounds.size.width;
    previewViewFrame.size.height = (420.0f / 320.0f) * previewViewFrame.size.width;
...
- (IBAction)handleLongPress:(UILongPressGestureRecognizer*)longPressGS {
    ...
    if (longPressGS.state == UIGestureRecognizerStateBegan) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            _captureButtonView.image = [UIImage imageNamed:@"camerainterface_takevideo"];
...
- (IBAction)handleTap:(UITapGestureRecognizer*)tapGS {
    ...
    if ([_recorder.sessionPreset isEqualToString:AVCaptureSessionPresetPhoto]) {
        ...
        if (tapGS.state == UIGestureRecognizerStateBegan) {
            ...
            [cameraShutter setBounds:CGRectMake(0.0, 0.0, self.containerView.frame.size.width, self.containerView.frame.size.height)];
...
{
    //_nextButton.hidden = NO;
    UIImage *squareImage = [self squareImageFromImage:image scaledToSize:self.containerView.frame.size.width];
    _captureButtonView.image = [UIImage imageNamed:@"camerainterface_takecamera"];
...
- (IBAction)switchCameraMode:(UIButton*)sender
{
    ...
    if ([_recorder.sessionPreset isEqualToString:AVCaptureSessionPresetPhoto]) {
        [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            [_flashModeButton setImage:[UIImage imageNamed:@"camerainterface_flash_off"] forState:UIControlStateNormal];
            _captureButtonView.image = [UIImage imageNamed:@"camerainterface_takevideo"];
...
- (IBAction)onShowGrid:(id)sender {
    self.gridImageView.hidden = !self.gridImageView.hidden;
}
    ...
    if ([_recorder.sessionPreset isEqualToString:AVCaptureSessionPresetPhoto]) {
        switch (_recorder.flashMode) {
            ...
            case SCFlashModeOff:
                ...
                [_flashModeButton setImage:[UIImage imageNamed:@"camerainterface_flash_on"] forState:UIControlStateNormal];
                break;
            case SCFlashModeOn:
            ...
            case SCFlashModeLight:
                ...
                _recorder.flashMode = SCFlashModeAuto;
                [_flashModeButton setImage:[UIImage imageNamed:@"camerainterface_flash_auto"] forState:UIControlStateNormal];
                break;
            default:
                break;
        }
...
- (IBAction)goToEditController:(id)sender {
    ...
    if(_mediaTypeImage) {
        PicMediaEditViewController *mediaViewController = [[PicMediaEditViewController alloc] initWithNibName:@"PicMediaEditViewController" bundle:nil];
        mediaViewController.mediaPath = _mediaPath;
        ...
        MediaEditViewController *mediaViewController = [[MediaEditViewController alloc] initWithNibName:@"MediaEditViewController" bundle:nil];
        
        mediaViewController.mediaPath = _mediaPath;
        mediaViewController.isMediaTypeImage = _mediaTypeImage;
        mediaViewController.recordSession = _recorder.recordSession;
