...
#define POPUP_WIDTH 204.0
#define POP_HEIGHT 170.0
#define ITEM_COUNT 9
#define ITEM_ROW_COUNT 3
...
-(void)commonInitWithImage:(UIImage *)image andTitle:(NSString *)aTitle andFrame:(CGRect) frame{
    ...
    title = [[UILabel alloc]initWithFrame:CGRectMake(-10, frame.size.height+1, frame.size.width+20, 40)];
...
-(instancetype)initWithItemImage:(NSArray *)items andTitles:(NSArray *)titleArray andActionArray:(NSArray *)actionArray addToViewController:(UIViewController*)sender {
    ...
    if (self) {
        ...
        contentView.backgroundColor = [UIColor colorWithWhite:0.1 alpha:0.3];
...
-(void)setUpPopItems {
    ...
    CGFloat pages =  (([popItems count] % ITEM_COUNT) == 0) ? [popItems count]/ITEM_COUNT : ([popItems count]/ITEM_COUNT) +1;
    ...
    if(_animationStyle == BTPopUPAnimateWithFade) {
        ...
        [UIView animateWithDuration:0.6
                         animations:^{
                             ...
                             contentView.frame = CGRectMake(self.frame.size.width/2-POPUP_WIDTH/2, SCREEN_SIZE.size.height, POPUP_WIDTH, POP_HEIGHT);
                         }];
    }else {
