...
- (void)showWithContentViewc:(UIViewController *)v andAnimated:(BOOL)animated withTopMargin:(float)topMargin {
    self.window = [[UIWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    self.window.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
    self.window.opaque = NO;
...
//    containerViewRect.origin.y = round(CGRectGetMidY(self.window.bounds)-CGRectGetMidY(containerViewRect));
    if(topMargin > 0) {
        containerViewRect.origin.y = topMargin;
    } else {
        containerViewRect.origin.y = ([[UIScreen mainScreen] bounds].size.height - containerViewRect.size.height) / 2.0f;
    }
    KGModalContainerView *containerView = [[KGModalContainerView alloc] initWithFrame:containerViewRect];
    containerView.modalBackgroundColor = self.modalBackgroundColor;
    containerView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleRightMargin|
