...
#define SGPOP_DEFAULT_MAX_HEIGHT    140
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        bgColor = [UIColor colorWithRed:26.0f/256.0f green:135.0f/256.0f blue:94.0f/256.0f alpha:0.95];
...
- (void)_setupConstraintsWithSuperView:(UIView *)view atPoint:(CGPoint)point
{
    ...
    width = width > view.bounds.size.width * 1 ? view.bounds.size.width * 1 : width;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_selectedIndex == indexPath.row) {
        cell.accessoryView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icon_ticked"]];
    }else {
        cell.accessoryView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icon_tick_blank"]];
    }
